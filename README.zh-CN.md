...
- 📊 **Insights 面板。** 针对当前筛选结果，实时展示 Top 机构 / 作者 / track 的图表，
  点击任意条形即可下钻筛选；点击 Top tracks 旁的图表按钮可查看各主题跨年份的趋势折线图。
- 🕸 **关系网络。** 针对当前结果集探索合作者网络和机构关系网络。
- ⭐ **分组、合集与标签。** 自定义会场分组（按系列）、把论文归入命名合集、给论文打任意
  标签，并保存筛选条件以便日后回到。所有数据都存在本地，可在设置面板中导出，并一览
  个人收藏全貌。
- 📖 **阅读状态与笔记。** 把论文标记为"想读 / 阅读中 / 已读"，并附上私人笔记；可按
  状态或"有笔记"进行筛选。登录后状态与笔记可跨设备同步。
- 🔍 **相似论文与"为你推荐"。** 打开任意论文的摘要，即可在全库中搜索相似论文。
  工具栏的"For you"按钮根据你已保存、打标签或做了笔记的论文进行个性化推荐——
  可以内联操作结果，也可以批量加入合集。
- 📤 **选择与导出。** 在列表中选中论文后，可一键复制 BibTeX 或下载 CSV，包含 DOI
  与出版元数据。
- 🔗 **分享与同步。** 用链接分享某个合集或整套配置。或登录 GitHub，在多设备间自动同步——数据存在仅你可访问的私有 gist 中。可查看完整配置版本历史并回滚到任意历史版本。使用无需账号，同步可选。
