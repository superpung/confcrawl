...
`Paper` 数据结构，再从官方详情页、书目与开放元数据源补全 DOI、摘要、出版信息和
开放访问链接。无需账号、没有后端——你的分组、合集、标签和已保存搜索都存在浏览器本地。你可以把所有内容导出为文件、用链接分享某个合集，或者可选地登录 GitHub、在多台设备间自动同步配置。同步完全可选，不登录也能完整使用 confer。
...
- 📤 **选择与导出。** 在列表中选中论文后，可一键复制 BibTeX 或下载 CSV，包含 DOI
  与出版元数据。
- 🔗 **分享与同步。** 用链接分享某个合集或整套配置。或登录 GitHub，在多设备间自动同步——数据存在仅你可访问的私有 gist 中。使用无需账号，同步可选。
- ⚡ **快且私密。** 单页预渲染，所有筛选都在客户端完成。支持深/浅色与强调色、键盘快捷键（`⌘K`、`⌘/`）以及移动端响应式布局。
...
- **适配器**各自只懂一个来源平台，但都产出*相同*的 `Paper` 结构；当来源记录链接到
  官方详情页时，适配器也会合并其中的信息。
